 ...
 */
enum DeviceOwner {
    DEVICE_OWNER_NONE = 0, DEVICE_OWNER_CHAMBER = 1, DEVICE_OWNER_BEER = 2
};
...
inline bool isAssignable(DeviceType type, DeviceHardware hardware) {
    return ((hardware == DEVICE_HARDWARE_PIN)
            && (type == DEVICETYPE_SWITCH_ACTUATOR
                    || type == DEVICETYPE_SWITCH_SENSOR
                    || type == DEVICETYPE_PWM_ACTUATOR
                    || type == DEVICETYPE_MANUAL_ACTUATOR))
...
inline DeviceOwner deviceOwner(DeviceFunction id) {
    return (id == 0) ? DEVICE_OWNER_NONE :
            (id >= DEVICE_BEER_FIRST) ? DEVICE_OWNER_BEER : DEVICE_OWNER_CHAMBER;
}
...
public:
    ...
    static void UpdateDeviceState(DeviceDisplay & dd, DeviceConfig & dc,
            char * val);
    ...
    static void printDevice(device_slot_t slot, DeviceConfig & config,
    ...
    static bool isDeviceValid(DeviceConfig & config, DeviceConfig & original,
            uint8_t deviceIndex);
    ...
    static void enumerateHardware(EnumerateHardware & spec,
            EnumDevicesCallback callback, DeviceCallbackInfo * callbackData);
...
private:
    ...
    static void enumerateOneWireDevices(EnumerateHardware & h,
            EnumDevicesCallback f, DeviceCallbackInfo * info);
    ...
    static void handleEnumeratedDevice(DeviceConfig & config, EnumerateHardware & h,
            EnumDevicesCallback callback, DeviceCallbackInfo * info);

    static void readTempSensorValue(DeviceConfig::Hardware hw, char * out);

    static void readValve(DeviceConfig::Hardware hw, char * out);

    static void writeValve(DeviceConfig::Hardware hw, uint8_t value);

    static void readPin(DeviceConfig::Hardware hw, char * out);

    static void writePin(DeviceConfig::Hardware hw, uint8_t value);

    static void readOneWirePin(DeviceConfig::Hardware hw, char * out);

    static void writeOneWirePin(DeviceConfig::Hardware hw, uint8_t value);

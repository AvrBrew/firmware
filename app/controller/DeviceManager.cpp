...
#else
            return new OneWireTempSensor(oneWireBus(config.hw.pinNr),
                    config.hw.address, config.hw.settings.sensor.calibration);
#endif
...
#else
            return new ActuatorOneWire(oneWireBus(config.hw.pinNr),
                    config.hw.address, config.hw.settings.actuator.pio,
                    config.hw.invert);
#endif
#endif
...
void DeviceManager::installDevice(DeviceConfig & config) {
    ...
    switch (dt) {
        ...
        case DEVICETYPE_TEMP_SENSOR: {
            ...
            if (s == NULL) {
                logErrorInt(ERROR_OUT_OF_MEMORY_FOR_DEVICE,
        ...
        case DEVICETYPE_PWM_ACTUATOR: {
            ...
            ActuatorDigital * newActuator = (ActuatorDigital *) createDevice(
                    config, dt);
...
bool DeviceManager::isDeviceValid(DeviceConfig & config,
    ...
    if (!(((owner == DEVICE_OWNER_BEER) && config.beer)
            || ((owner == DEVICE_OWNER_CHAMBER) && config.chamber)
            || ((owner == DEVICE_OWNER_NONE) && !config.beer && !config.chamber))) {
        logErrorIntIntInt(ERROR_INVALID_DEVICE_CONFIG_OWNER, owner, config.beer,
                config.chamber);
    ...
    if (!isAssignable(dt, config.deviceHardware)) {
        logErrorIntInt(ERROR_CANNOT_ASSIGN_TO_HARDWARE, dt,
                config.deviceHardware);
...
void DeviceManager::printDevice(device_slot_t slot, DeviceConfig & config,
        const char * value, Print & p) {
    char buf[17];
    DeviceType dt = deviceType(config.deviceFunction);
...
inline bool matchAddress(uint8_t * detected, uint8_t * configured,
        uint8_t count) {
    if (!configured[0]) {
        return true;
    }
...
inline void DeviceManager::readValve(DeviceConfig::Hardware hw, char * out) {
    OneWire * bus = oneWireBus(hw.pinNr);
...
inline void DeviceManager::writeValve(DeviceConfig::Hardware hw,
        uint8_t value) {
    OneWire * bus = oneWireBus(hw.pinNr);
    DS2408 && hwDevice = DS2408();
    hwDevice.init(bus, hw.address);
...
inline void DeviceManager::writeOneWirePin(DeviceConfig::Hardware hw,
        uint8_t value) {
    OneWire * bus = oneWireBus(hw.pinNr);
    ActuatorOneWire pin(bus, hw.address, hw.settings.actuator.pio, hw.invert);
    pin.write(value);
}

inline void DeviceManager::readOneWirePin(DeviceConfig::Hardware hw,
        char * out) {
    OneWire * bus = oneWireBus(hw.pinNr);
    ActuatorOneWire pin(bus, hw.address, hw.settings.actuator.pio, hw.invert);
...
void DeviceManager::handleEnumeratedDevice(DeviceConfig & config,
        EnumerateHardware & h, EnumDevicesCallback callback,
        DeviceCallbackInfo * info) {
    if (h.function
            && !isAssignable(deviceType(DeviceFunction(h.function)),
                    config.deviceHardware)) {
...
void DeviceManager::UpdateDeviceState(DeviceDisplay & dd, DeviceConfig & dc,
        char * val) {
    ...
    if (dd.write >= 0) {
        ...
        if (dt == DEVICETYPE_SWITCH_SENSOR) {
            sprintf_P(val, STR_FMT_U,
                    (unsigned int) ((SwitchSensor *) *ppv)->sense() != 0); // cheaper than itoa, because it overlaps with vsnprintf
            ...
            sprintf_P(val, STR_FMT_U,
                    (unsigned int) ((ActuatorDigital *) *ppv)->isActive() != 0);
        } else if (dt == DEVICETYPE_PWM_ACTUATOR) {
